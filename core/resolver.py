"""
core/resolver.py
Turns a signature DB entry into a live, verified address for the attached
process, preferring an AOB scan (version-independent) and only falling
back to the historical RVA when the pattern is too weak to scan safely.

Resolution order for each entry:
  1. If we have a per-build cache hit (keyed by exe SHA256) for this
     signature id, use it directly -- instant, no scanning needed.
  2. Otherwise, if the entry has an `aob` pattern, scan the module for it.
     A *unique* hit wins.
  3. Otherwise (or if the scan was ambiguous/empty), fall back to
     addr(historical_rva) -- i.e. module_base + historical_rva.
  4. Whatever address wins, verify it against `verify_bytes` before
     trusting it. Mismatches are reported, not silently patched over --
     calling code decides whether to proceed.

This mirrors exactly what a careful human doing this manually in Cheat
Engine would do: scan first, fall back to the known offset, always check
the bytes before touching anything.
"""
...
class Resolved:
    ...
    method: str          # "cache" | "scan" | "rva" | "missing"
    verified: bool
    detail: str = ""
