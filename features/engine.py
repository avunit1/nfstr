"""
features/engine.py
Generic dispatcher that turns a resolved signature + a patch_type into an
actual memory effect. This is the only place that "knows" how each
patch_type behaves, so the GUI layer just calls engine.enable(id) /
engine.disable(id) and doesn't care whether that means flipping a jcc,
NOPing bytes, installing a code cave, or writing through a pointer chain.

patch_type reference
---------------------
nop            overwrite `hook_len` bytes with 0x90 at enable, restore
               the real original bytes (captured live) at disable.
byte_write     write `enabled_bytes` at enable, `disabled_bytes` (or the
               live-captured original) at disable.
jcc_invert     flip a conditional jump's condition (e.g. je<->jne) in place.
               Auto-detects short (2-byte) vs near (6-byte) encoding from
               the live bytes -- see core/jcc.py -- so a wrong assumption
               in the signature DB about which form the compiler used
               can't corrupt the jump.
jcc_force_jmp  convert a conditional jump into an unconditional one that
               lands on the exact same target the original condition
               would have jumped to, re-deriving the correct rel8/rel32
               displacement for the new instruction (see core/jcc.py's
               module docstring for why this can't be done by reusing
               the original displacement bytes).
codecave       install/uninstall a precomputed code-cave detour (see
               core/codecave.py). Some codecaves take a live-adjustable
               value (e.g. desired FPS, traffic density) -- for those,
               `value_field` names which offset inside the cave body
               holds an immediate operand we patch before installing.
pointer_write  resolve a pointer chain and write a value through it
               once (used for one-shot actions like swapping the current
               car). Not a persistent toggle -- the GUI gives this its
               own dedicated control rather than a checkbox.
pointer_toggle like pointer_write, but a real on/off toggle: enable()
               writes `on_value` through the pointer chain, disable()
               writes `off_value`. Used for simple pointer-addressed
               flags (e.g. HUD visibility) where, unlike the register-
               relative codecave case, we already have a stable static
               pointer chain and don't need a code cave at all.
freeze         like pointer_write, but re-applied on a timer while the
               toggle is on (used for "no damage", "infinite nitrous"
               style effects where the game keeps resetting the value).
cave_field_freeze
               like `freeze`, but the base address isn't fixed -- it's a
               pointer captured by a *different* signature's data-slot
               codecave (see `cave_ref` on the signature entry), read
               fresh on every tick via CodeCaveEngine.read_data_pointer().
               This is how framerate-unlock actually works: the GameTime
               cave exposes the current struct's address once gameplay
               reaches it, and this writes MaxVariableFps at
               pointer + field_offset every tick from then on. The
               referenced cave is installed automatically if needed.
"""
...
class FeatureEngine:
    def __init__(self, resolver: SignatureResolver, mem: SafeMemory,
        ...
        self._cave_freeze: dict[str, dict] = {}  # sid -> {cave_ref, field_offset, packed}
    ...
    def enable(self, sig: dict, **kwargs) -> bool:
        ...
        # cave_field_freeze has no hook site of its own -- it rides on a
        # *different* signature's cave (sig["cave_ref"]) -- so it doesn't
        # go through the normal "resolve my own address first" gate.
    ...
    def disable(self, sig: dict) -> bool:
        ...
        try:
            if patch_type == "nop":
                ...
                ok = True  # pointer_write has nothing persistent to undo
        except Exception as e:
    ...
    def _do_jcc_invert(self, sig, addr) -> bool:
        """Flip a conditional jump's condition. See core/jcc.py -- the form
        (short/near) is detected from the live bytes, never assumed."""
        current = self.mem.read_bytes(addr, 6)  # read enough for either form
        if current is None:
            return False
    ...
    def _do_jcc_force_jmp(self, sig, addr) -> bool:
        """Convert a conditional jump into an unconditional one landing on
        the same target. See core/jcc.py for why the displacement has to
